...
public class MyLinkedList {
	...
	private MyLinkedList reverse(ListNode node) {
      ...
      if (node == null) { // base case
         ...
         MyLinkedList reversedSubList = reverse(node.next); //node.next
      
         reversedSubList.addLast(node.data); //recursive call
   ...
   //addlastmethod
    public void addLast(Object data) {
      ...
      ListNode cur = this.head;
         while (cur.next != null){ //walk through
             cur = cur.next;
         }
      ListNode nn = new ListNode(data);
      cur.next = nn; //here i struggled because i forgot to do cur.next instead of cur and it would give me nullpointer because .next.next
      this.size++;// size++
	 ...
	 */
	private ListNode reverse(ListNode first, ListNode second) { //ListNode prev, ListNode subHead
      ListNode h = null;
      if (second == null) { //basecase
         return first;
      }
      else {
         h = reverse(second, second.next); //second points to first node in the sublist
         second.next = first;
         first.next = null;
      }
